...
import json
from models.predict import BatchRequest
...
async def predict_categories(batch: BatchRequest):
    results = []
    for item in batch.items:
        category = predict_category(item.title)
        results.append({"title": item.title, "detail": item.detail, "purchasedAt" : item.purchasedAt, "quantity" : item.quantity, "cost" : item.cost, "img" : item.img, "category": category})
    return results